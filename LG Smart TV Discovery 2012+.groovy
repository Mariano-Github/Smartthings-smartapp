 ...
 * -Open The App, click on done until error network appears.
 * -Close smattthings and reopen it. 
 * -In Smartapp it smrtapp appears as pending configuration. Clock on it.
 * -Select the TV app found, just find 1. click on "done" until the key code appears on the TV screen, take note of it.
 * -Open IDE, my smartapps, open this app. In the (line 142 def pairingKey ="XXXXXX") of code, write the copied code TV key. 
 * -Repeat the process and the TV control will be created. 
    ...
    log.debug "input Key-0"
     return dynamicPage(name:"televisionAuthenticate", title:"LG TV Search Started!", nextPage:"", install:true){
        section("We sent an pairing request to your TV. Please enter the pairing key and click Done."){
          log.debug "input Key"
          input (name:"pairingKey", type:"string", description: "960781", defaultValue:"960781", required:false, title:"Pairing Key", multiple:false) 
        }
    }
}
...
//**** write here the key shown on your TV ****
  def pairingKey ="XXXXXX"
// ********************************************
  //log.debug "deviceSettings[1]= ${deviceSettings[1]}, deviceSettings[2]= ${deviceSettings[2]}"
